...
public class FleshKincaidLogic {
	...
	private boolean isEndOfSentence(String word) {
		...
		if (wordCountForOneWordSentenceTracking == 1
				&& punctuationThatEndSentences.contains(
		...
		// first remove an ending quote if there is one
			...
			for (int i = 0; i < punctuationThatEndSentences.size(); i++) {
				currentPunctuation = String.valueOf(punctuationThatEndSentences
						.get(i));
	...
	private int countSyllables(String word) {
		...
		if (word.indexOf("www") > 0 || word.indexOf("http") > 0
				|| word.indexOf("@") > 0 || word.indexOf(".co") > 0) {
			return 1;
		}
			...
			if (isVowel(stringArray[i])) {
				...
				//character is a constanent
					...
					while (i < wordLength - 1) {
						...
						if (isVowel(stringArray[i]) == false
								&& (Character.isLetter(stringArray[i]) == true)) {
							syllableCount++;
	...
	private double getFleschReadingEaseNotIncludingFragments() {
		...
		fleschReadingEase = 206.835
				- (1.015 * ((double) wordsInFileNotIncludingFragments / (double) sentencesInFile))
				- (84.6 * getSyllablesPerWordInFile());
